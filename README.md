...
## Notifying everyone the app's link changed

A Site Admin can click **📣 Notify link change** at the top of Users & access to email every active user the current `APP_URL`, with a short "please update your bookmark" message (`renderAppUrlChangedEmail` in `app/lib/email.ts`, sent by `POST /api/users/notify-link-change`). Built for exactly the situation that prompted it: the app moved off `tasks.flenner.at` onto `https://tasks-ai.onrender.com` after its DNS turned out to be permanently blocked (see `render.yaml`'s `APP_URL` comment), and existing users needed to hear about it. The idempotency key is tied to the current `APP_URL`, so clicking it twice in a row for the same address is a harmless no-op — but the button stays useful for the next time the address changes, e.g. once `task-ai.flenner.at` is set up.
