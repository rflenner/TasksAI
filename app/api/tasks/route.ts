...
import { checklistJustAdvanced } from "../../lib/checklist";
...
// expected case.
// created is computed once, up front, so due (below) can default off the
// exact same value that ends up stored — never a separately-computed
// "now" a moment later that could, in theory, land on the next day.
function values(input:Input){const created=String(input.created||new Date().toISOString());return{subject:String(input.subject||"").trim(),description:String(input.description||""),owner:String(input.owner||"").trim(),collaborators:cleanList(input.collaborators),recipients:cleanList(input.recipients),due:resolveDueDate(input.due,created),source:String(input.source||""),topic:String(input.topic||""),project:String(input.project||""),recurringMeeting:String(input.recurringMeeting??input.recurring_meeting??""),status:String(input.status||"Open"),priority:(["Low","Medium","High"] as const).includes(input.priority as "Low"|"Medium"|"High")?input.priority as string:"Low",created,updates:Array.isArray(input.updates)?input.updates:[],checklist:Array.isArray(input.checklist)?input.checklist:[],externalSource:input.externalSource?String(input.externalSource):null,externalId:input.externalId?String(input.externalId):null,accountId:input.accountId?String(input.accountId):null,accountName:input.accountName?String(input.accountName):null,opportunityId:input.opportunityId?String(input.opportunityId):null,opportunityName:input.opportunityName?String(input.opportunityName):null,citationUser:input.citationUser?String(input.citationUser):null,citationQuote:input.citationQuote?String(input.citationQuote):null,meetingId:input.meetingId?String(input.meetingId):null,ownerContactId:input.ownerContactId?String(input.ownerContactId):null,recipientContactIds:input.recipientContactIds&&typeof input.recipientContactIds==="object"?input.recipientContactIds:{}}}
...
export async function PATCH(request:Request){const invalid=requireSameOrigin(request);if(invalid)return invalid;const actor=await currentActor();if(!actor)return Response.json({error:"Sign in required"},{status:401});const body=await request.json() as Input;if(!body.id)return Response.json({error:"Task id is required"},{status:400});const[existing]=await getDb().select().from(tasks).where(eq(tasks.id,body.id)).limit(1);if(!existing)return Response.json({error:"You cannot change this task"},{status:403});
 ...
 // Checking a checklist item is the same "someone's making progress"
 // signal a posted update already is — see checklistJustAdvanced's own
 // comment for why only a newly-CHECKED item counts, not an unchecked one.
 const checklistAdvanced=checklistJustAdvanced(existing.checklist,rebuilt.checklist);
 const advanced=updatesGrew||checklistAdvanced;
 const finalStatus=advanced?autoAdvanceStatus(existing.status,true,null):rebuilt.status;
 const next={...rebuilt,status:finalStatus,id:body.id};if(!canWriteTask(existing,actor)||!canWriteTask(next,actor))return Response.json({error:"You cannot change this task"},{status:403});const closedAt=finalStatus!=="Closed"?null:existing.status==="Closed"?existing.closedAt:new Date();const[task]=await getDb().update(tasks).set({...rebuilt,status:finalStatus,closedAt}).where(eq(tasks.id,body.id)).returning();await register(task);await recordActivity(task.id,actor.name,describeChanges(existing,task));await noteAssignments(task.id,newlyAssignedPeople(existing,task));const justClosed=existing.status!=="Closed"&&finalStatus==="Closed";if(justClosed||advanced)await notifySlackOnTaskChange(task,actor.name,justClosed?"closed":"update");return Response.json({task,dimensions:await dimensions()})}
