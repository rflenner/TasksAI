...
export default function LoginClient({ initialError }: { initialError: string }){
 const[email,setEmail]=useState(""),[code,setCode]=useState(""),[password,setPassword]=useState(""),[step,setStep]=useState<"start"|"email"|"code"|"password">("start"),[state,setState]=useState<"ready"|"working">("ready"),[error,setError]=useState(initialError);
 ...
 const signInWithPassword=async()=>{if(!email.includes("@")||!password||state==="working")return;setState("working");setError("");const response=await fetch("/api/auth/password-login",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email,password})});if(response.ok){window.location.href="/";return}setState("ready");const data=await response.json().catch(()=>({}));setError(data.error||"Incorrect email or password.")};
 ...
 return <main className="onboard"><section className="onboard-card"><div className="onboard-logo">Task <b>AI</b></div><p>SECURE SIGN-IN</p><h1>Welcome back</h1>
  {step==="start"
   ?<div className="onboard-form">
     ...
     <button type="button" style={{background:"none",border:0,color:"#697181",cursor:"pointer",font:"inherit",padding:0,textAlign:"center"}} onClick={()=>{setStep("password");setError("")}}>Use a password instead</button>
    </div>
   :step==="password"
   ?<><p>Sign in with the password your Task AI admin gave you.</p><div className="onboard-form"><label className="onboard-field"><span>Email address</span><input type="email" autoComplete="email" value={email} onChange={event=>setEmail(event.target.value)} placeholder="you@company.com"/></label><label className="onboard-field"><span>Password</span><input type="password" autoComplete="current-password" value={password} onChange={event=>setPassword(event.target.value)} onKeyDown={event=>{if(event.key==="Enter")void signInWithPassword()}} placeholder="••••••••••••"/></label>{error&&<p className="onboard-error">{error}</p>}<button className="onboard-primary" disabled={state==="working"||!email.includes("@")||!password} onClick={()=>void signInWithPassword()}>{state==="working"?"Signing in…":"Sign in"}</button><button style={{background:"none",border:0,color:"#697181",cursor:"pointer",font:"inherit",padding:0,textAlign:"left"}} type="button" onClick={()=>{setStep("start");setPassword("");setError("")}}>← Back</button></div></>
   :step==="email"
