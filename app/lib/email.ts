...
export function renderPendingTasksEmail(input: { firstName: string; appUrl: string; myTasks: PendingTaskLine[]; delegatedTasks: PendingTaskLine[]; recentlyClosed: PendingTaskLine[]; overdueCount?: number }) {
  ...
  </style></head><body style="margin:0;background:#f5f7fa;font-family:Arial,Helvetica,sans-serif"><table role="presentation" width="100%"><tr><td align="center" class="tai-outer" style="padding:40px 16px"><table role="presentation" width="100%" style="max-width:600px;background:#fff;border-radius:16px"><tr><td class="tai-inner" style="padding:28px 34px;border-bottom:1px solid #e7ebef"><span style="font-size:28px;font-weight:800;color:#173f76;vertical-align:middle">Task</span> <span style="display:inline-block;padding:5px 6px;border-radius:5px;background:#ffa614;color:#fff;font-size:18px;font-weight:800;line-height:1;vertical-align:middle">AI</span> <span style="font-size:28px;font-weight:800;color:#173f76;vertical-align:middle">– Pending Tasks Update</span></td></tr><tr><td class="tai-inner" style="padding:38px 34px"><div style="color:#102f59;font-size:14px;font-weight:700">Hi ${esc(firstName)},</div><div style="margin-top:4px;color:#102f59;font-size:14px;font-weight:700">you have ${esc(summary)}${overdueClause}.</div><div style="margin-top:4px;color:#5b6577;font-size:13px">Please review below or open Task AI.</div><div style="margin-top:4px;color:#9299a3;font-size:12px">Click a task to open it in Task AI, or use its <b>Add an update</b> button to update it right from this email — no sign-in needed.</div>${sectionsHtml}<a href="${esc(openLinkUrl)}" style="display:inline-block;margin-top:24px;padding:14px 22px;border-radius:8px;background:#173f76;color:#fff;font-weight:700;text-decoration:none">Open Task AI</a></td></tr></table></td></tr></table></body></html>`;
...
export function renderNewTasksEmail(input: { firstName: string; appUrl: string; newTasks: PendingTaskLine[] }) {
  ...
  </style></head><body style="margin:0;background:#f5f7fa;font-family:Arial,Helvetica,sans-serif"><table role="presentation" width="100%"><tr><td align="center" class="tai-outer" style="padding:40px 16px"><table role="presentation" width="100%" style="max-width:600px;background:#fff;border-radius:16px"><tr><td class="tai-inner" style="padding:28px 34px;border-bottom:1px solid #e7ebef"><span style="font-size:28px;font-weight:800;color:#173f76;vertical-align:middle">Task</span> <span style="display:inline-block;padding:5px 6px;border-radius:5px;background:#ffa614;color:#fff;font-size:18px;font-weight:800;line-height:1;vertical-align:middle">AI</span> <span style="font-size:28px;font-weight:800;color:#173f76;vertical-align:middle">– New Assignments</span></td></tr><tr><td class="tai-inner" style="padding:38px 34px"><div style="color:#102f59;font-size:14px;font-weight:700">Hi ${esc(firstName)},</div><div style="margin-top:4px;color:#102f59;font-size:14px;font-weight:700">you have been assigned ${count} new task${count === 1 ? "" : "s"} in Task AI.</div><div style="margin-top:4px;color:#5b6577;font-size:13px">Please click on the tasks below to review them.</div><div style="margin-top:4px;color:#9299a3;font-size:12px">Click a task to open it in Task AI, or use its <b>Add an update</b> button to update it right from this email — no sign-in needed.</div>${newSection.html}<a href="${esc(openLinkUrl)}" style="display:inline-block;margin-top:24px;padding:14px 22px;border-radius:8px;background:#173f76;color:#fff;font-weight:700;text-decoration:none">Open Task AI</a></td></tr></table></td></tr></table></body></html>`;
...
// sendManualNotify. First cut quoted the note in its own box with no task
// context at all; Rizan's follow-up: "use the templates we have, but add
// the text above" — so this now reuses taskCard(), the exact same card
// every digest email already renders (status pill, due badge, tags,
...
export async function sendWithResend(message: { to: string; cc?: string; subject: string; html: string; text: string; idempotencyKey?: string; replyTo?: string }) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from, to: [message.to], ...(message.cc ? { cc: [message.cc] } : {}), ...(message.replyTo ? { reply_to: [message.replyTo] } : {}), subject: message.subject, html: message.html, text: message.text, tags: [{ name: "app", value: "task-ai" }] }),
  });
